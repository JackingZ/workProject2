<template>
  ...
  >
    ...
    >
      <el-link
        v-permission="['warehouseOrderFinancialAdd']"
        :underline="false"
        style="float: left"
        type="primary"
        @click="addInvoice"
      >
        {{ custId === orderCustomerInfo.parentId ? '新增应收款' : '新增应付款' }}
      </el-link>
      <el-link
        :underline="false"
        type="primary"
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <el-button
              v-if="itemV.status !== '1' && !itemV.isEdit && $checkPermission(['warehouseOrderFinancialAudit'])"
              type="text"
            ...
            <el-button
              v-if="!itemV.isEdit && !itemV.isAdd && itemV.ifVerify === 0 && itemV.status !== '3' && $checkPermission(['warehouseOrderFinancialEdit'])"
              type="text"
...
import { getWinOrderByCode } from '@/api/orderlist-at-warehouse'
...
export default class extends Vue {
  ...
  private show(row: any, orderType: any) {
    this.rowDetail = row
    this.getCurrencyList()
    this.getCostitemList()
    this.getSelfCurrency()
    if (row.type === '3') {
      const seller = this.rowDetail.seller || {}
      const buyer = this.rowDetail.buyer || {}
      this.orderCustomerInfo = {
        custId: buyer.id,
        custName: buyer.name,
        parentId: seller.id,
        parentName: seller.name
      }
      this.getInvoiceList()
      this.getCustType()
    } else {
      this.loading = true
      getWinOrderByCode({ code: row.code, orderType }).then((res: any) => {
        this.loading = false
        this.orderCustomerInfo = res.data
        this.getInvoiceList()
        this.getCustType()
      }).catch(() => {
        this.loading = false
  ...
  private getCustType() {
    getCustType([this.orderCustomerInfo.custId]).then((res: any) => {
      const data = res.data || []
  ...
  private getInvoiceItem() {
    const invoice: any = {
      serviceSubtypeId: this.rowDetail.type,
      currencyId: 'RMB', // 所选货币
      currencyName: '人民币',
      ...
      exRate: '',
      amount: '',
      agentId: this.orderCustomerInfo.parentId,
      agentName: this.orderCustomerInfo.parentName,
      clientId: this.orderCustomerInfo.custId,
      clientName: this.orderCustomerInfo.custName,
      serviceTypeId: this.service.id,
      serviceTypeName: this.service.name,
      invoiceType: '',
      orderId: this.rowDetail.id,
      orderCode: this.rowDetail.code,
  ...
  private getInvoiceList(openId?: any) {
    const params: any = {
      orderId: this.rowDetail.id,
      enterpriseId: [
        this.orderCustomerInfo.parentId
      ],
      serviceTypeId: this.service.id
    }
    this.loading = true
