...
export default class extends Vue {
  ...
  private getTrailerOrderList() {
    const params = {
      model: {
        sellerId: this.form.agentId,
        type: this.searchForm.type,
        soCodes: this.searchForm.number,
        orderType: 0,
        customerId: this.form.clientId,
  ...
  private getFullOrderList() {
    ...
    const params = {
      model: {
        buyerId: cType === '0' ? undefined : this.form.clientId,
        type: this.searchForm.type,
        code: this.searchForm.number,
        sellerId: cType === '0' ? this.form.clientId : this.form.agentId,
        endDate: this.searchForm.orderTime[1] + ' 23:59:59',
        startDate: this.searchForm.orderTime[0] + ' 00:00:00',
  ...
  private getDetailsById(id: string) {
    ...
    getInvoiceById(params).then(async(res: any) => {
      const { items, clientProperty, ...form } = res.data
      this.selfCurrency = {
      ...
      try {
        this.form = {
          ...
          clientType: clientProperty ? clientProperty.substring(0, 1) : '1'
        }
        this.setCustomerType()
