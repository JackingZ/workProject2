      ...
      <div>
        ...
        >
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span v-if="form.custType === '0'">聚货通</span>
                <span v-else-if="form.custType === '3'">供应商</span>
                <span v-else-if="form.custType === '2' && scope.row.subtype === '1'">一级代理</span>
                <span v-else-if="form.custType === '2' && scope.row.subtype === '2'">二级代理</span>
                <span v-else-if="form.custType === '1' && scope.row.subtype === '0'">直客</span>
                <span v-else-if="form.custType === '1' && scope.row.subtype === '1'">同行</span>
                <span v-else>--</span>
              </template>
            </el-table-column>
            <el-table-column
...
export default class extends Vue {
  ...
  private formRules = {
    id: [{ required: true, message: '请款申请单号不能为空', trigger: 'blur' }],
    ...
    payAmount: [{ required: true, message: '请款金额不能为空', trigger: 'blur' }],
    selfAmount: [{ required: true, message: '本币金额不能为空', trigger: 'blur' }],
    applyTime: [{ required: true, message: '请选择请款申请单日期', trigger: 'change' }],
  ...
  get invoiceFilter() {
    const data = (this as any).invoicesData.filter((it: any) => it.tag !== 'X')
    let num = 0
    data.map((it: any) => {
  ...
  get invoicesTable() {
    return this.invoicesData.filter((it: any) => it.tag !== 'X')
  }
  get customerList() {
  ...
  private getRequestById(id: string) {
    ...
    getRequestById({
      ...
      .then(async(res: any) => {
        const data = res.data || {}
        const { invoiceEntityList, custType, ...form } = data
        this.form = {
          ...form,
        ...
        this.invoicesData = invoiceEntityList || []
  ...
  private handleSave(): void {
    ...
    editForm.validate((valid: any) => {
      ...
      params.invoiceIdList = this.invoicesData.filter((it: any) => it.tag !== 'X').map((it: any) => it.id)
      this.currencyList.map((it: any) => {
  ...
  private handleInvoice(): void {
    ...
    this.isInvoice = true
    this.invoiceArr = this.invoicesData.filter((it: any) => it.tag !== 'X')
    this.page = 1
    this.size = 10
    this.total = 0
  }
  ...
  private invoiceSelectionConfirm() {
    ...
    this.selectType = ''
    this.keyWord = ''
    const oldArr = this.invoicesData.filter((it: any) => it.tag !== 'X')
    ...
    this.invoicesData = resArr
  }
  private handleDelete(index: any, str: string): void {
    // 删除
    if (str === '2') {
      this.invoicesData[index].tag = 'X'
    }
  }
}
</script>
