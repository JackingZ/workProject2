        ...
        >
          <el-row>
            ...
            >
              ...
              <el-select
                v-if="searchForm.comType === '2' || searchForm.comType === '4' || searchForm.comType === '5'"
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  ...
                  <span v-else-if="scope.row.clientProperty === '3'">供应商</span>
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    v-if="scope.row.ifVerify === 0 && scope.row.status !== '3' && $checkPermission(['finaReceiveUpdate'])"
                    type="text"
                  ...
                  <el-button
                    v-if="scope.row.ifVerify === 0 && $checkPermission(['finaReceiveDel'])"
                    type="text"
                  ...
                  <el-button
                    v-permission="['finaReceiveDetail']"
                    type="text"
                  ...
                  <el-button
                    v-if="scope.row.status !== '1' && $checkPermission(['finaReceiveAudit'])"
                    type="text"
                  ...
                  <!-- <el-button
                    v-if="scope.row.status === '1' && $checkPermission(['finaReceiveRollBack'])"
                    type="text"
                    @click="rollbackInvoice(scope.row.id)"
                  >
                    回滚
                  </el-button> -->
                  <!-- <el-button
...
import detail from '@/views/financial-management/invoice-receivable/detail.vue'
...
export default class extends Vue {
  ...
  private rollbackInvoice(id: any) {
    this.$confirm('请确认是否状态回滚?', '温馨提示', {
      ...
      this.loading = true
      rollbackInvoice({ id }).then(() => {
        this.loading = false
        this.$message.success('状态回滚成功')
        this.handleSearch()
      }).catch(() => {
        this.loading = false
      })
  ...
  private handleExport2() {
    ...
    const params: any = {
      idList: this.options.map((it: any) => it.id)
    }
      ...
      .then((res: any) => {
        ...
        if (this.options.length) {
          this.options.map((a: any) => {
            const item = this.tableData.find((it: any) => it.id === a.id || `${it.id}` === a.id)
            if (item) {
              this.$nextTick(() => {
                const root: any = this.$refs.tableSelect
