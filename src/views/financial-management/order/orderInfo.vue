<template>
  ...
  >
    <div class="top-option">
      <el-link
        v-if="$checkPermission(['addInvoiceOrder'])"
        :underline="false"
        type="primary"
        @click="addInvoice"
      >
    ...
    >
      ...
      >
        ...
        >
          <el-button
            v-if="itemV.status !== '1' && !itemV.isEdit && $checkPermission(['auditInvoiceOrder'])"
            type="text"
          ...
          <el-button
            v-if="!itemV.isEdit && !itemV.isAdd && itemV.ifVerify === 0 && itemV.status !== '3' && $checkPermission(['editInvoiceOrder'])"
            type="text"
...
export default class extends Vue {
  ...
  private getInvoiceItem() {
    const invoice: any = {
      serviceSubtypeId: (this as any).rowDetail.type,
      currencyId: 'RMB', // 所选货币
      currencyName: '人民币',
      ...
      exRate: '',
      amount: '',
      agentId: (this as any).orderCustomer.parentId,
      agentName: (this as any).orderCustomer.parentName,
      clientId: (this as any).orderCustomer.id,
      clientName: (this as any).orderCustomer.name,
      serviceTypeId: '05',
      serviceTypeName: '专线',
      invoiceType: '',
      orderId: (this as any).rowDetail.id,
      orderCode: (this as any).rowDetail.code,
      invDate: dayjs().format('YYYY-MM-DD HH:mm:ss'),
      remark: '',
      clientProperty: (this as any).orderCustomer.type + '-' + (this as any).orderCustomer.subtype,
      status: '0',
      items: [],
      isOpen: true,
      isEdit: true,
    ...
    this.invoiceList = this.invoiceList.map((e: any) => {
      if (e.id === invoice.id) {
        e.items = e.items.map((d: any, i: any) => {
          if (i === itemIndex) {
            d.amount = (costitem.quantity || 0) * (costitem.price || 0)
  ...
  private getInvoiceList(openId?: any) {
    const { code, seller = {} } = (this as any).rowDetail
    const params: any = {
      orderCode: code,
      enterpriseId: [
        (this as any).orderCustomer.parentId
      ],
      serviceTypeId: '05'
    }
    this.loading = true
