<template>
  <div style="display: flex;flex-flow: row nowrap;min-height: inherit;">
              ...
              >
                <div style="padding-right: 10px;">
                  ...
                  >
                    ...
                    <el-button
                      v-permission="['customsQuoteQuery']"
                      type="primary"
                      @click="getPriceList('')"
                    >
                      筛选
    ...
    >
      ...
      >
        <div style="display: flex;align-items: center;justify-content: space-between">
            ...
            >
              <span slot="label">
                ...
                <el-tooltip
                  ...
                  slot="label"
                  :content="selectService.label + '/' + selectService.csId"
                  placement="top"
                  effect="light"
                >
          ...
          >
            <el-button
              v-if="!isEdit && isView && (selectCard.auditing === 1 || selectCard.auditing === 3) && $checkPermission(['customsQuoteEdit'])"
              type="text"
              size="mini"
              @click="editRow"
            >
            ...
            <el-popover
              v-if="selectCard.auditing !== 4 && !isEdit && isView && $checkPermission(['customsQuoteAudit'])"
            ...
            <el-button
              v-if="(!isView || isEdit) && $checkPermission(['customsQuoteAdd', 'customsQuoteEdit'])"
              type="text"
              size="mini"
              @click="cancelEdit"
            >
            ...
            <el-button
              v-if="!isEdit && isView && (selectCard.auditing === 1 || selectCard.auditing === 3) && $checkPermission(['customsQuoteDel'])"
              type="text"
              size="mini"
              style="margin: 0 10px"
              @click="deleteRow(1)"
          ...
          >
            <el-button
              v-if="$checkPermission(['customsQuoteCopy'])"
              type="text"
              size="mini"
              @click="copyPrice"
            >
    ...
    <hs-modal
      ref="hsModal"
      @close="() => serviceChange(selectService.id, 'select')"
    />
    <transfer-modal
      ref="transferModal"
    ...
    <transition name="el-fade-in-linear">
      <div
        v-show="showAgentPriceModel && showAddForm && custType === '0' && $checkPermission(['customsQuoteQuotation'])"
        v-drag
        class="agent-price"
      >
        <el-tooltip
    ...
    >
      ...
      <li
        v-if="(showMenuRow.auditing === 1 || showMenuRow.auditing === 3) && $checkPermission(['customsQuoteDel'])"
        class="contextmenu-item"
        @click="deleteRow(2)"
      >
        删除
...
import {
  ...
  getCscCostitem,
  getOrgList
} from '@/api/price-customs-clearance'
import dayjs from 'dayjs'
import transferModal from '@/views/price-management/price-customs-clearance/components/transferModal.vue'
import AgentPrice from '@/views/price-management/price-customs-clearance/components/agentPrice.vue'
import costModal from '@/views/price-management/price-customs-clearance/components/costModal.vue'
import hsModal from '@/views/service-configuration/customs-clearance/components/transferModal.vue'
...
export default class extends Vue {
  ...
  private showTransferModal() {
    if (!this.selectService.id) {
      this.$message.warning('请选择服务')
      return
    }
    (this.$refs.hsModal as any).show(this.selectService, this.selectService.countryName + '-' + this.selectService.subtypeName + '-' + this.selectService.csId)
  }
  private showCostModal() {
    if (!this.selectService.id) {
      this.$message.warning('请选择服务')
      return
    }
    (this.$refs.costModal as any).show(this.selectService)
  }
  ...
  private showMenu(row: any, e: MouseEvent) {
    if (!(this as any).$checkPermission(['customsQuoteCopy', 'customsQuoteGet', 'customsQuoteDel'])) return
  ...
  private getSuperiorPrice() {
    const list: string[] = ['serviceId', 'subtypeId']
    this.yAxis = []
    this.xAxis = []
    if (this.validateService(list) === 0) {
      ...
      getSuperiorPrice(params).then((res: any) => {
        ...
        if (res.success && res.data && res.data.length) {
          ...
          // if (this.superiorPriceList.length === 1) {
          //   const superiorPriceId = this.superiorPriceList[0].id
          //   this.selectForm.refId = superiorPriceId
  ...
  private getStandardPrice() {
    if (!this.selectForm.ifStandard) {
      const list: string[] = ['serviceId', 'subtypeId', 'toCustSubtype']
      if (this.validateService(list) === 0) {
        const offerObject = this.getOfferObject()
        const params = {
          toSubtype: offerObject.toCustSubtype,
  ...
  private async viewPrice(card: any, isCopy?: boolean) {
    if (!(this as any).$checkPermission(['customsQuoteCopy', 'customsQuoteGet'])) return
    ...
    this.selectForm.selectCountryId = this.selectService.countryId
    if (card.refId) {
      if (this.custType === '2') {
  ...
  private getParams() {
    ...
    const params: any = {
      ...
      country: this.selectService.countryName,
    ...
    return new Promise((resolve: any, reject: any) => {
      ...
      getOrgList(params).then((res: any) => {
        if (res.success) {
          ...
          this.xAxis = result.map((item: any) => {
            return {
              ...item,
  ...
  private getYAxis() {
    ...
    getCscCostitem(params).then((res: any) => {
      this.loadingTable = false
      if (res.success) {
        const result = res.data
        if (result.length === 0) {
          this.$message.warning('请设置报价成本项目')
          return
        }
        this.yAxis = result.map((item: any) => {
          ...
          this.xAxis.forEach((e: any) => {
            tamp[e.value] = undefined
          })
          return tamp
        })
      }
  ...
  private async getSubtypeList() {
    this.selectLoading = true
    await getServiceSubtypeList({ typeId: '03' }).then((res: any) => {
  ...
  private showModel() {
    const list: string[] = ['subtypeId', 'serviceId', 'toCustSubtype']
    const offerObject = this.getOfferObject()
    if (this.validateService(list) === 0) {
      const modelRef: any = this.$refs.transferModal
      modelRef.showModel(this.selectService, this.selectForm.associatedAgents, this.formLabel, offerObject)
    }
  }

  private transferModalSave(selectKeys: any[], selectRows: any[]) {
    this.selectForm.associatedAgents = selectKeys
    if (this.selectCard.status !== '1' && this.selectCard.auditing === 4) {
      const params = this.getParams()
      params.id = this.selectCard.id
      params.fromSubtype = this.selectCard.fromSubtype
      params.fromType = this.selectCard.fromType
